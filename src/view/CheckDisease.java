...
public class CheckDisease extends JPanel {
	...
	public CheckDisease() {
		...
		JLabel lblCheckPatient = new JLabel("Check Disease :");
		...
		comboBox.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				...
				if(pat!=null)
				{
					if(MainWindow.getInstance().getKindofuser()==0) // if the user is doctor
					{
						if(doc.checkDisease(pat)) // doctor check the patient
						{
					...
					else if(MainWindow.getInstance().getKindofuser()==1) // if the user is nurse
					{
						if(nur.checkDisease(pat)) // nurse check the patient
						{
		...
		comboBoxNurse.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				try {
					...
					if(doc!=null)
						throw new WrongField("Plesase don't select doctor and nurse togther");
					if(nur.checkDisease(pat))
					{
						JOptionPane.showMessageDialog(null, "The Patient " + pat.getFname() + " " + pat.getLname() + " check by the nurse " + nur.getFname() + " " + nur.getLname(), "Check", JOptionPane.INFORMATION_MESSAGE);
		...
		add(comboBoxNurse);
		if(MainWindow.getInstance().getKindofuser()==2)
		{
		...
		else if(MainWindow.getInstance().getKindofuser()==1)
		{
			...
			comboBoxNurse.setEditable(false);
			comboBoxDoctor.setEditable(false);
			addPatients(1);
		}
		else if(MainWindow.getInstance().getKindofuser()==0)
		{
