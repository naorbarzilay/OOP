...
public class Doctor extends HospitalUser implements Serializable {
	...
	public boolean checkPatient(Patient p) {
		if(p == null)
			...
			Date now = new Date();
			updateShiftCounter();
			PatientReport report = new PatientReport(p, this,now,
					p.getDis(), p.getsDepartment(), p.checkCondition());
	...
	/**
	 * a method that classifies a patient disease to viral disease or chronic disease
	 * @param patient
	 * @return true/false
	 */
	public boolean checkDisease(Patient p) {
		...
		Disease d = p.getDis();
		updateShiftCounter();
		if(d.getSymptoms().contains(Symptoms.DIFFICULTY_BREATHING) && 
				d.getSymptoms().contains(Symptoms.FEVER))
